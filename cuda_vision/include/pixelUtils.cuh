...
static __device__ scalar_t get_value_channel(
    ...
    return gray[batch_off + c * channel_off + y * w + x];
}
...
static __device__ void set_value_channel(
    ...
    int channel_off = h * w;
    int batch_off = channel_off * num_ch * blockIdx.z;
    gray[batch_off + c * channel_off + y * w + x] = value;
}
...
#define _GET(x, y) (IS_OUT(x, y, w, h) ? 0.0 : gray[batch_off + y * w + x])
